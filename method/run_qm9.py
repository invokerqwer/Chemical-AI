...
class run_qm9():
    ...
    def run(self, device, train_dataset, valid_dataset, test_dataset, model, loss_func, evaluation, epochs=500, batch_size=32, vt_batch_size=32, lr=0.0005, lr_decay_factor=0.5, lr_decay_step_size=30, weight_decay=0, 
        energy_and_force=False, p=100, save_dir='', log_dir='',tar='U0',train=True):
        r"""
        ...
        for epoch in range(1, epochs + 1):
            ...
            scheduler.step()
            fn = 'checkpoint'+'/savori5'+str(tar)+'.pt' 
            torch.save(model.state_dict(), fn)
            print('save') 
            file1=open('mae'+'/mae16'+'_'+str(tar)+'.txt','a')
            print(train_mae,valid_mae,test_mae,file=file1)
            file1.close()  
    ...
    def train(self, model, optimizer, train_loader, energy_and_force, p, loss_func, device):
        ...
        fn = 'checkpoint/qm9/save4_zpve.pt'
        torch.save(model.state_dict(), fn)
        print('save')
